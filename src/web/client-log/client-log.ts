...
async function sendPOEEchoesMessage(msg: string) {
  const botToken = '1604482498:AAHe4HR6LjbbEQxBLxGTtoSb69NKB9bUfAw'
  const chatId = -412940651

  const result = await sendTelegramMessage(botToken, chatId, msg)

  console.log(`Send telegram result:`, result)
}

async function sendTelegramMessage(botToken: string, chatId: number, msg: string) {
